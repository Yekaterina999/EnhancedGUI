	...
	"attack-move":
	{
		"execute": function(position, action, selection, queued, pushFront)
		{
			...
			if (Engine.HotkeyIsPressed("session.attackmoveUnit")){
				targetClasses = { "attack": ["Ranged", "Infantry", "Cavalry"] };
				print("Attack enemy soldiers")
			}

			else if (Engine.HotkeyIsPressed("session.attackSupport")){
				targetClasses = { "attack": ["Female", "Support"]};
				print("Attack support");
			}


			else {
				targetClasses = { "attack": ["Units"] };
				print("Attack all");
			}
