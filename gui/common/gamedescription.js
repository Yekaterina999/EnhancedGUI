...
function formatPlayerInfo(playerDataArray, playerStates)
{
	...
	if (playerDescriptions.observer){
		let numberOfObservers = playerDescriptions.observer.length;
		print(numberOfObservers)

		teamDescription.push(sprintf(translate("%(team)s:\n%(playerDescriptions)s"), {
			"team": '[font="sans-bold-14"]' + translatePlural("Observer (1)", "Observers (" + numberOfObservers.toString() + ")", numberOfObservers) + "[/font]",
