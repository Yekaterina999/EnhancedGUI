...
function isAttackMovePressed()
{
	return Engine.HotkeyIsPressed("session.attackmove") ||
		Engine.HotkeyIsPressed("session.attackmoveUnit") || Engine.HotkeyIsPressed("session.attackSupport");
}
...
function positionUnitsFreehandSelectionMouseUp(ev)
{
	...
	if (Engine.HotkeyIsPressed("session.attackmoveUnit")){
		let targetClasses = { "attack": ["Ranged", "Infantry", "Cavalry"] };
		print("Walk-Attack enemy soldiers");
	}
	else if (Engine.HotkeyIsPressed("session.attackSupport")){
		let targetClasses = { "attack": ["Support", "Female"] };
		print("Walk-Attack just support units");
	}
	else {
		let targetClasses = { "attack": ["Unit"] };
		print("Walk-Attack all");
	}
